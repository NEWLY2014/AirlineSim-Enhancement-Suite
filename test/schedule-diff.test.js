...
test('diff detects added, removed, time, equipment and weekday changes without row-order noise',async t=>{
    const p=setup(t);
    ...
    const result=await compare(p,a,b);
    ...
    for(const field of ['days','aircraft']){
        ...
        assert.equal((await compare(p,capture({1:frequency()}),next)).changes[0].kind,'Changed');
    }
});

test('legacy snapshots compare frequencies without inventing new timing changes',async t=>{
    const p=setup(t),old=frequency();delete old.services;
    let result=await compare(p,capture({1:old}),capture({1:frequency()}));
    assert.equal(result.changes.length,0);assert.equal(result.limited,true);
    const changed=frequency();changed.paxFreq=4;
    assert.equal((await compare(p,capture({1:old}),capture({1:changed}))).changes.length,1);
});

test('comparison dialog supports filters and search and uses text for airline names',async t=>{
    const p=setup(t);
