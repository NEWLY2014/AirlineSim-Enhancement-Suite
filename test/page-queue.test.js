...
test('queue applies randomized 50–70 ms dispatch gaps without waiting for loading',async()=>{
    for(const random of [0,0.5,0.999999]) {
        const q=queue(random);await q.message('enqueue','a');await q.message('enqueue','b','open',2);
        assert.equal(q.opened.length,1);
        const delay=50+Math.floor(random*21);q.advance(delay-1);await q.message('poll','b','open',2);assert.equal(q.opened.length,1);
...
test('price grant shares dispatch spacing but does not hold the queue during refresh',async()=>{
    ...
    q.advance(50);await q.message('poll','open','open',2);assert.equal(q.opened.length,1);
...
test('queue notifies the exact waiting document after the preceding permit completes',async()=>{
    ...
    q.advance(50);assert.equal((await q.message('poll','b','price',2)).state,'running');
});
...
test('loading completion does not reset the dispatch clock',async()=>{
    ...
    q.advance(30);await q.message('poll','b','open',2);assert.equal(q.opened.length,2);
});
...
test('read requests from different pages share completion-to-start gaps of 50–70 ms without opening tabs',async()=>{
    for (const random of [0,0.5,0.999999]){
        ...
        const gap=50+Math.floor(random*21);q.advance(gap-1);
        assert.equal((await q.message('poll','r2','read',2,read)).state,'queued');
...
test('failed or cancelled read releases the queue with a cooldown before price operations',async()=>{
    ...
    await q.message('cancel','r','read',1,read);q.advance(49);assert.equal((await q.message('poll','p','price',2)).state,'queued');
    q.advance(1);assert.equal((await q.message('poll','p','price',2)).state,'running');
});


test('same-tab navigation shares the 50–70 ms queue interval with tab opening',async()=>{
    for(const random of [0,0.5,0.999999]) {
