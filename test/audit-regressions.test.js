...
async function planner(t,id='123',arrival='8') {
    const form=`<form><select name="existingNumber:numbers:numbers_body:input"><option value="10">AA 100</option></select>${Array.from({length:7},(_,i)=>`<input type="checkbox" name="days:daySelection:${i}:ticked"><input type="checkbox" name="segmentsContainer:segments:0:fixedArrivalSelection:${i}:fixedArrival"><select name="segmentsContainer:segments:0:newArrivals:${i}:newArrival:hours"><option value="${arrival}">${arrival}</option></select><select name="segmentsContainer:segments:0:newArrivals:${i}:newArrival:minutes"><option value="30">30</option></select>`).join('')}<input type="submit" name="button-submit"></form>`;
