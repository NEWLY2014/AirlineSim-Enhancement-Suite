...
function sortDashboardTable(table: JQuery, columnClass: string, number: boolean | number | undefined) {
    AESDashboardTable.sort(table, columnClass, !!number);
}
    ...
    if (tbody) {
        Array.from(tbody.rows).forEach(function(row) {
            ...
            aggregates.forEach(function(aggregate, index) {
                ...
                let value = typeof raw === 'number' ? raw : AESDashboardTable.parseNumber(raw);
                if (!isNaN(value)) {
    ...
    $('tbody tr', table).each(function() {
        ...
        filters.forEach(function(filter) {
            ...
            if (column.number) {
                ...
                value = value ? AESDashboardTable.parseNumber(value) : 0;
            }
...
function setDefaultRouteManagementSettings() {
    settings.routeManagement = AESDashboardDefaults.getDefaultRouteManagementSettings();
}
...
function ensureRouteManagementSettings() {
    ...
    const defaultSettings = AESDashboardDefaults.getDefaultRouteManagementSettings();
...
function displayCompetitorMonitoringAirlinesTableOptions(table?: JQuery, compAirlinesSchedule: Record<string, AESModel.DashboardSchedule> = {}, mainDiv: JQuery = $(), removeCompetitorFromIndex: (id: string) => void = () => {}, columns: AESModel.DashboardColumn[] = []) {
    ...
    refreshBtn.on('click',async () => {
        ...
        async function refreshRow(row: AESModel.DashboardRow) {
            ...
            if (sort) AESDashboardTable.sort(table!,sort.columnClass,sort.number,sort.ascending);
        }
    });
    if (!table) {
...
function setDefaultCompetitorMonitoringSettings() {
    let columns = AESDashboardDefaults.getDefaultCompetitorMonitoringColumns();
...
function ensureCompetitorMonitoringSettings() {
    ...
    let defaultColumns = AESDashboardDefaults.getDefaultCompetitorMonitoringColumns();
